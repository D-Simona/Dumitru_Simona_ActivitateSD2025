...
void afisareVector(struct Telefon* vector, int nrElemente) {
	...
	//trb sa parcurgem vectorul printr-o instructiune for
...
struct Telefon* copiazaPrimeleNElemente(struct Telefon* vector, int nrElemente, int nrElementeCopiate) {
	//copiem intr-un vector nou pe care il vom returna cu primele nrElementeCopiate
	...
	for (int i = 0; i < nrElementeCopiate; i++) {
		...
		strcpy_s(vectorNou[i].producator, strlen(vector[i].producator) + 1, vector[i].producator); //deep-copy  pt producator (char *)
	}
...
void copiazaTelefoaneScumpe(struct Telefon* vector, char nrElemente, float pretMinim, struct Telefon** vectorNou, int* dimensiune) {
	//parametrul prag(pretMinim) poate fi modificat in functie de tipul atributului ales pentru a indeplini o conditie
...
int main() {
	...
	//ne declaram un vector de telefoane = un pointer la structura Telefon si sa ii alocam spatiu
