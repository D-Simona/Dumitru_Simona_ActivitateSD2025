...
void filtreazaHeap(Heap heap, int pozitieNod) {
	...
	if (pozFiuStanga<heap.nrMasini && heap.vector[pozMax].pret < heap.vector[pozFiuStanga].pret) {
		pozMax = pozFiuStanga;
	}
	if (pozFiuDreapta<heap.nrMasini && heap.vector[pozMax].pret < heap.vector[pozFiuDreapta].pret) {
	...
	if (pozMax != pozitieNod) {
		...
		//2 * pozMax + 1 <= heap.nrMasini - 1; de aici il scoatem pe pozMax
