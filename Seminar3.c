//hash table (tabela de dispersie) -> hash = criptare care nu se decripteaza
//coliziune = acelasi cod hash pt 2 elemente diferite => metoda chaining (adaugam noduri noi =>lista simpla/dubla inlantuita)
...
//creare structura pentru un nod dintr-o lista simplu inlantuita (recomandata)
...
void adaugaMasinaInLista(Nod** cap, Masina masinaNoua) {
	//adauga la inceput in lista primita o noua masina pe care o primim ca parametru
...
int main() {
	...
	Masina mSofer = getMasinaDupaSoferSiID(ht, "Ionescu", 5);
	if (m.id != -1) {
		printf("\n Masina cautata dupa Sofer si Id: \n");
		afisareMasina(mSofer);
	}

	printf("\n Preturi medii pe cluster: \n");
	int nrClustere = 0;
