...
void adaugaMasinaInLista(Nod* cap, Masina masinaNoua) { //o apelam doar cand avem coliziune
...
int calculeazaHash(const char* numeSofer, int dimensiune) {
	...
	//functie de calculare a hashului - cod ASCII
	int suma = 0;
	for (int i = 0; i < strlen(numeSofer); i++) {
		suma += numeSofer[i]; //ia codul ASCII si il adauga la suma
	}
...
void inserareMasinaInTabela(HashTable hash, Masina masina) {
	...
	//este determinata pozitia pe care trb sa inseram prin aoelul fct calculeazaHash 
	//si se realizeaza inserarea pe pozitia respectiva
...
Masina getMasinaDupaNumeSofer(HashTable ht, const char* numeCautat) {
	...
	//trebuie sa modificam numele functiei (era cheie si noi am folosit numele soferului drept cheie)
